...
int Window::Initialise()
{
	...
	//CREAR VENTANA
	mainWindow = glfwCreateWindow(width, height, "Practica 05: Optimizacon y Carga de Modelos", NULL, NULL);
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F) //COFRE HACIA ARRIBA
	{
	...
	if (key == GLFW_KEY_C) //COFRE HACIA ABAJO
	{
	...
	if (key == GLFW_KEY_G) //AUTO Y RUEDAS HACIA ADELANTE
	{
	...
	if (key == GLFW_KEY_V) //AUTO Y RUEDAS HACIA ATRÁS
	{
		if (theWindow->articulacion2 > 45.0)
		{
		}
